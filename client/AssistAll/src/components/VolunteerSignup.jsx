  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
          ...formData,
          role: 'volunteer',
          govtId: govtIdFile ? govtIdFile.name : 'uploaded_id.jpg',
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="w-full max-w-lg bg-[#0a0a0a] border border-[#222] p-8 rounded-[32px] shadow-2xl relative z-10 animate-in zoom-in duration-300">
            ...
            <div className="flex justify-center gap-2 mb-8 mt-2">{[1,2,3].map(i => <div key={i} className={`h-1 w-12 rounded-full transition-all ${step >= i ? 'bg-blue-600' : 'bg-[#222]'}`}></div>)}</div>
                ...
                <div className="space-y-6 animate-in slide-in-from-right">
                    <div className="grid grid-cols-3 gap-3">
                        <button onClick={() => setFormData({...formData, serviceSector: 'transport'})} className={`p-4 rounded-2xl border flex flex-col items-center gap-2 transition ${formData.serviceSector === 'transport' ? 'bg-blue-600 border-blue-400 text-white' : 'bg-[#111] border-[#222] text-gray-500 hover:bg-[#1a1a1a]'}`}>
                            <Car size={24}/><span className="text-[10px] font-bold uppercase">Transport</span>
                        </button>
                        <button onClick={() => setFormData({...formData, serviceSector: 'medical'})} className={`p-4 rounded-2xl border flex flex-col items-center gap-2 transition ${formData.serviceSector === 'medical' ? 'bg-green-600 border-green-400 text-white' : 'bg-[#111] border-[#222] text-gray-500 hover:bg-[#1a1a1a]'}`}>
                            <Stethoscope size={24}/><span className="text-[10px] font-bold uppercase">Medical</span>
                        </button>
                        <button onClick={() => setFormData({...formData, serviceSector: 'companionship'})} className={`p-4 rounded-2xl border flex flex-col items-center gap-2 transition ${formData.serviceSector === 'companionship' ? 'bg-purple-600 border-purple-400 text-white' : 'bg-[#111] border-[#222] text-gray-500 hover:bg-[#1a1a1a]'}`}>
                            <HeartHandshake size={24}/><span className="text-[10px] font-bold uppercase">Helper</span>
                        </button>
                    </div>
                    ...
                    <div className="bg-[#111] p-5 rounded-2xl border border-[#222] space-y-4">
                        ...
                        <div className="flex items-center justify-between p-3 bg-[#0a0a0a] rounded-xl border border-[#222] cursor-pointer hover:border-blue-500 transition" onClick={() => document.getElementById('govtId').click()}>
                            <div className="flex items-center gap-3"><Shield className="text-blue-500" size={18}/><span className="text-sm font-medium text-gray-300">Government ID</span></div>
                        ...
                        {formData.serviceSector === 'transport' && (
                            <>
                                <div className="flex items-center justify-between p-3 bg-[#0a0a0a] rounded-xl border border-[#222] cursor-pointer hover:border-blue-500 transition" onClick={() => document.getElementById('license').click()}>
                                    <div className="flex items-center gap-3"><FileText className="text-orange-500" size={18}/><span className="text-sm font-medium text-gray-300">Driving License</span></div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <input placeholder="Vehicle Model" className="bg-[#0a0a0a] border border-[#222] p-3 rounded-xl text-sm focus:border-blue-500 outline-none" onChange={e => setFormData({...formData, vehicleModel: e.target.value})}/>
                                    <input placeholder="Plate No." className="bg-[#0a0a0a] border border-[#222] p-3 rounded-xl text-sm focus:border-blue-500 outline-none" onChange={e => setFormData({...formData, vehicleNumber: e.target.value})}/>
                                </div>
                            </>
                        )}
                        ...
                        {formData.serviceSector === 'medical' && (
                            <div className="flex items-center justify-between p-3 bg-[#0a0a0a] rounded-xl border border-[#222] cursor-pointer hover:border-green-500 transition" onClick={() => document.getElementById('medCert').click()}>
                                <div className="flex items-center gap-3"><Briefcase className="text-green-500" size={18}/><span className="text-sm font-medium text-gray-300">Nursing Certificate</span></div>
            ...
            {step === 3 && (
                <div className="space-y-6 animate-in slide-in-from-right text-center">
                    ...
                    <div><h3 className="text-xl font-bold">Final Verification</h3><p className="text-gray-500 text-sm mt-2 max-w-xs mx-auto">Please join the video call with our admin to verify your <b>{formData.serviceSector}</b> credentials.</p></div>
                    <a href="https://meet.google.com/fsv-htsz-srx" target="_blank" className="flex items-center justify-center gap-3 w-full bg-[#111] hover:bg-[#222] text-blue-500 font-bold py-4 rounded-xl border border-blue-900/30 transition"><Video size={20}/> Join Interview</a>
                    <div className="relative"><Key className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500"/><input placeholder="Enter Admin Code" className="w-full bg-[#111] border border-[#222] rounded-xl py-4 pl-12 pr-4 text-center font-mono text-xl tracking-widest focus:border-green-500 outline-none transition" onChange={e => setFormData({...formData, adminCode: e.target.value})} /></div>
                    <button onClick={handleSubmit} disabled={loading} className="w-full bg-green-600 text-black font-bold py-4 rounded-xl hover:bg-green-500 transition shadow-[0_0_20px_rgba(22,163,74,0.3)]">{loading ? <Loader2 className="animate-spin mx-auto"/> : "Complete Registration"}</button>
                </div>
            )}
        </div>
    </div>
