...
// ✅ FIX: Accepting specific props for each action
const LandingPage = ({ onLogin, onUserSignup, onVolunteerSignup }) => {
    ...
    <div ref={containerRef} className="h-screen w-full bg-black text-white font-sans overflow-y-auto overflow-x-hidden selection:bg-green-500 selection:text-black relative scroll-smooth">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 px-6 py-4 flex justify-between items-center ${scrollY > 20 ? 'bg-black/90 backdrop-blur-xl border-b border-white/5' : 'bg-transparent'}`}>
        ...
        <div className="hidden md:flex gap-3">
            {/* ✅ FIX: Log In -> Goes to Login Page */}
            <button onClick={onLogin} className="px-5 py-2 text-sm font-bold text-white hover:text-green-400 transition">Log In</button>
            {/* ✅ FIX: Get Started -> Goes to User Signup */}
            <button onClick={onUserSignup} className="bg-white text-black px-6 py-2.5 rounded-full text-sm font-bold hover:bg-gray-200 transition transform hover:scale-105">Get Started</button>
        </div>
      ...
      {isMobileMenuOpen && (
          <div className="fixed inset-0 z-40 bg-black/95 backdrop-blur-xl pt-24 px-6 flex flex-col gap-6 md:hidden animate-in slide-in-from-top-10">
              ...
              <div className="flex flex-col gap-4 mt-4">
                  ...
                  <button onClick={onUserSignup} className="w-full bg-green-600 text-black py-4 rounded-xl font-bold">Get Started</button>
              </div>
          </div>
      )}
      ...
      <section id="home" className="relative pt-40 pb-20 px-6 flex flex-col items-center text-center min-h-[90vh] justify-center z-10">
        <div className="max-w-5xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-center items-center w-full">
                {/* ✅ FIX: Hero Button -> Goes to User Signup */}
                <button onClick={onUserSignup} className="w-full md:w-auto px-8 py-4 bg-green-600 text-black rounded-full font-bold text-lg flex items-center justify-center hover:bg-green-500 hover:scale-105 transition group">
                    Find Help Now <ArrowRight className="ml-2 group-hover:translate-x-1 transition-transform"/>
                </button>
                {/* ✅ FIX: Volunteer Button -> Goes to Volunteer Signup */}
                <button onClick={onVolunteerSignup} className="w-full md:w-auto px-8 py-4 bg-transparent border border-white/20 text-white rounded-full font-bold text-lg hover:bg-white/5 transition flex items-center justify-center gap-2">
      ...
      <section id="volunteer" className="py-24 px-6 bg-neutral-900 border-y border-white/5 relative overflow-hidden">
          <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center relative z-10">
              <div>
                  ...
                  {/* ✅ FIX: Volunteer CTA -> Goes to Volunteer Signup */}
                  <button onClick={onVolunteerSignup} className="mt-12 bg-white text-black px-10 py-4 rounded-full font-bold text-lg hover:bg-gray-200 transition flex items-center gap-2 group">Apply Now <ChevronRight className="group-hover:translate-x-1 transition"/></button>
              </div>
